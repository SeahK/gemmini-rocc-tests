...
// mvin and mvout
#define gemmini_priority_mvin(dram_addr, spad_addr, cols, rows, priority) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, dram_addr, ((uint64_t)(priority) << 62) | ((uint64_t)(rows) << (ADDR_LEN + 16)) | ((uint64_t)(cols) << ADDR_LEN) | (spad_addr), k_MVIN)

#define gemmini_priority_mvin2(dram_addr, spad_addr, cols, rows, priority) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, dram_addr, ((uint64_t)(priority) << 62) | ((uint64_t)(rows) << (ADDR_LEN + 16)) | ((uint64_t)(cols) << ADDR_LEN) | (spad_addr), k_MVIN2)

#define gemmini_priority_mvin3(dram_addr, spad_addr, cols, rows, priority) \
  ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, dram_addr, ((uint64_t)(priority) << 62) | ((uint64_t)(rows) << (ADDR_LEN + 16)) | ((uint64_t)(cols) << ADDR_LEN) | (spad_addr), k_MVIN3)
...
#define gemmini_loop_ws(I, J, K, pad_I, pad_J, pad_K, A, B, D, C, A_stride, B_stride, D_stride, C_stride, A_transpose, B_transpose, full_C, low_D, ex_accumulate, weightA, priority) \
  { \
...
#define gemmini_loop_conv_ws(batch_size, in_dim, in_channels, out_channels, out_dim, pool_out_dim, stride, padding, kernel_dim, kernel_dilation, pool_size, pool_stride, pool_padding, batches, porows, pocols, pochs, krows, kcols, kchs, lpad, rpad, upad, dpad, plpad, prpad, pupad, pdpad, orows, ocols, weights, output, bias, input, no_bias, no_pool, downsample, wrot180, input_dilated, trans_output_1203, trans_weight_1203, trans_weight_0132, trans_input_3120, priority) \
  { \
    ...
    ROCC_INSTRUCTION_RS1_RS2(XCUSTOM_ACC, ((trans_input_3120) << 5) | ((trans_weight_0132) << 4) | ((trans_weight_1203) << 3) | ((trans_output_1203) << 2) | ((wrot180) << 1) | (no_bias), \
      ((uint64_t)(priority) << 3) | ((input_dilated) << 2) | ((downsample) << 1) | (no_pool), k_LOOP_CONV_WS) \
  }
...
static void sp_tiled_matmul_os(const elem_t * A, const elem_t * B, const void * D, void * C,
        ...
        uint8_t weightA, uint8_t priority) {
...
// This function runs a tiled matrix multiplication, with automatically
// calculated tiling factors
static void tiled_matmul_auto_priority(size_t dim_I, size_t dim_J, size_t dim_K,
        const elem_t* A, const elem_t* B,
        const void * D, void * C,
        size_t stride_A, size_t stride_B, size_t stride_D, size_t stride_C,
        scale_t A_scale_factor, scale_t B_scale_factor, scale_acc_t D_scale_factor,
                        ...
                        for (int kcol = 0; kcol < kernel_dim; kcol += kcols) {
                            ...
                            for (int kch = 0; kch < in_channels; kch += kchs) {
                                ...
                                sp_tiled_conv_A_stride(
                                    ...
                                    no_bias, no_pool, downsample, input_dilated, priority);
                            }
                        }
                    }
                }
...
static void tiled_conv_A_stride_auto_priority(
        int batch_size, int in_dim, int in_channels,
        int out_channels, int out_dim,
        int stride, int input_dilation, int kernel_dilation, int padding, int kernel_dim,
        bool wrot180, bool trans_output_1203, bool trans_input_3120,
...
static void sp_tiled_resadd_priority(const size_t I, const size_t J,
    ...
    for (size_t i = 0; i < I; i += DIM) {
        for (size_t j = 0; j < J; j += blocks * DIM) {
            ...
            gemmini_priority_mvin(A_dram_addr, A_sp_addr, cols, rows, priority);
        }
    }
    ...
    for (size_t i = 0; i < I; i += DIM) {
        for (size_t j = 0; j < J; j += blocks * DIM) {
            ...
            gemmini_priority_mvin2(B_dram_addr, B_sp_addr, cols, rows, priority);
        }
    }
        ...
        for (size_t j = 0; j < J; j += tile_J) {
            ...
            elem_t * c = C + i * J + j;
				...
				if(priority == 0)
					...
					sp_tiled_resadd_priority(I_tile, J_tile,
							A_scale, B_scale, a, b, c,
							J, J, J,
							relu, priority);
        }
...
// with priority bit encoding
static void tiled_resadd_auto_priority(const size_t I, const size_t J,
        const scale_t A_scale,
        const scale_t B_scale,
        const acc_scale_t C_scale,
        const elem_t * A,
